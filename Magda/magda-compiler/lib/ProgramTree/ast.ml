...
(*** Field Declarations ***)
and field_decl = { (* The record [field_decl] contains the following fields:
...
and parameter_decl = { (* The record [parameter_decl] contains the following fields:
...
and init_param = { (* The record [init_param] contains the following fields:
...
(*** Source Parameters ***)
and source_param = { (* The record [source_param] are the parameters in input/output.
...
and variable_decl = { (* The record [variable_decl] contains the following fields:
...
(*** Polymorphism ***)
and polymorphism_param = {(* The record [polymorphism_param] is used to define polymorphic parameters in [mixin_decl]. 
...
(*** Method Body ***)
and method_body = {(* The record [method_body] is the body of a [NewMethod] or [OverrideMethod].
...
(*** Ini Module super ***)
and ini_module_super = { (* The record [ini_module_super] is the supercall of the inimodule. It contains the following fields:
...
(*** Ini Module Body ***)
and ini_module_body = { (* The record [ini_module_body] is the body of an [ini_module_decl].
...
and ini_module_decl = { (* The record [ini_module_decl] is the declaration of an ini module.
  The record contains the following fields:
...
(*** Mixin Declarations ***)
and mixin_decl = {(* The record [mixin_decl] contains the following fields:
...
and abstract_method = {(* The record [abstract_method] is used to express the abstract methods of a mixin. It contains the following fields:
...
and new_method = {(* The record [new_method] is used to express the new methods of a mixin. It contains the following fields:
...
and override_method = {(* The record [override_method] is used to express the override methods of a mixin. It contains the following fields:
...
and object_creation = {(* The record [object_creation] is used to express the object creation expression. It contains the following fields:
...
and field_selection = {(* The record [field_selection] is used to express the field selection expression. It contains the following fields:
...
and method_call = {(* The record [method_call] is used to express the method call expression. It contains the following fields:
...
and while_instruction = {(* The record [while_instruction] is used to express the while instruction. It contains the following fields:
...
and if_instruction = {(* The record [if_instruction] is used to express the if instruction. It contains the following fields:
...
and ini_module_super_instruction = {(* The record [ini_module_super_instruction] is used to express the super call of an ini module. It contains the following fields:
...
and mixin_expr_application = {(* The record [mixin_expr_application] is used to express the application of a mixin to a mixin expression. It contains the following fields:
...
and field_lvalue = {(* The record [field_lvalue] is used to express the field lvalue. It contains the following fields:
...
(* Mixin Expressions ***)
and mixin_expr =(* A [mixin_expr] is used to express the type of declarations such as variables, parameters, fiels and return.
...
(* LValues ***)
and lvalue =(* A [lvalue] is a reference to a location in memory.
...
(* Instructions ***)
and instruction =(* An [instruction] is one of the operations expressed in the guards.
...
(*** Declarations ***)
and global_declaration = (* A [global_declaration] is either a [mixin_decl] or a [let_decl] statement *)
...
(*** Method Declaration ***)
and method_declaration = (* A [method_declaration] is a bodyless abstract [AbstactMethod] or a concrete [NewMethod] or an override of an existing mixin [OverrideMethod]*)
